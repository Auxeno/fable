...
class MultiHeadAttention(nnx.Module):
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        mask = -1e9 * jnp.triu(jnp.ones((s, s), dtype=x.dtype), k=1)  # (b, s, s)
        logits += mask
...
class GroupQueryAttention(nnx.Module):
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        -----
        b = batch_size, s = seq_len, d = dim, n = num_heads, h = head_dim,
        g = num_groups, m = heads_per_group

        Example
        -------
        num_heads = 8, num_groups = 2 -> heads_per_group = 4
        """
        ...
        mask = -1e9 * jnp.triu(jnp.ones((s, s), dtype=x.dtype), k=1)  # (s, s)
        ...
        x = jnp.einsum("bgmst,bgsh->bsgmh", attention, v)  # (b, s, g, m, h)
